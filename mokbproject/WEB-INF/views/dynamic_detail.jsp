...
<html>
  <head>
    <base href="<%=basePath%>">
		...
		<link rel="stylesheet" type="text/css" href="css/dynamic_detail.css"/>
	...
	<body>
		...
		<div class="mui-content">
			...
			<!--动态列表-->
			<div class="detail" id="shangpingList"></div>
		</div>
		...
		<script type="text/javascript">
			...
			function findall(a,x){
			 ...
			 function dianZan(){
	    		...
	    		if(jsonNewid11==null){
	    			var jsonNewid={"jsonNewid1":[],"jsonNewid2":[],"jsonNewid3":[]};
	      			$.cookie('jsonNewid', JSON.stringify(jsonNewid), { expires:10, path:  "/" });//过期时间7天
	    		}
	    		var jsonread=$.cookie('jsonNewid');
	     		var jsonNewid= JSON.parse(jsonread);//转对象
	   			var index=findall(jsonNewid.jsonNewid2,parseInt(news_id));
	   			...
	   			if(index.length>0){   
	    			...
	    			function thumbsUp(data){
	      				if(data.state==0){
	     					...
	     					$("#zan").html(zainum);
	       					jsonNewid.jsonNewid2.push(parseInt(news_id));
	       					$.cookie('jsonNewid', JSON.stringify(jsonNewid), { expires:7, path:  "/" });//过期时间7天
	      				}
	       			}
			 ...
			 function findNews(data){    
		        ...
		        var Ahtml = '<div class="title">' +
		         ...
		         function onload11(){
	              ...
	              //返回上级
				function toBack(){
					window.location="<%=basePath%>HomeController/dynamic";
				}
	      	</script>
	</body>
