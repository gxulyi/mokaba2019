...
jsonStringCokie.setMaxAge(1700);//28分钟左右
...
<html>
	...
	<head>
		...
		<style type="text/css">
			.order-form-box{position: relative;clear: all;overflow: hidden;padding: 5px;}
			.custom-table-view-cell{border-bottom: 1px solid #E5E5E5;}
			.line-height-35{line-height: 35px;}
			.mui-input-row input[type=button]{position: relative;clear: all;overflow: hidden;width: 50%;color: white;background-color: green;border: none;}
			.card-tip{line-height: 40px;text-align: center;font-size:14px;}
			.card-tip span{color: dodgerblue;}
			table{border-collapse:collapse;width: 100%;font-size:12px;}
			table, td, th{border:1px solid #62bca3;text-align: center;color: #62bca3;}
		</style>
				...
				<div class="order-form-box">
					<ul class="mui-table-view">
						<li class="custom-table-view-cell">
							<div id="shangping" style="position: relative;clear: all;overflow: hidden;padding: 5px;"></div>
						</li>
						<li class="custom-table-view-cell">
							<div style="position: relative;clear: all;overflow: hidden;padding: 5px;">
								订&nbsp;&nbsp;单&nbsp;&nbsp;号：<span><%=orderNo %></span>
							</div>
						</li>
						<li class="custom-table-view-cell">
							<div style="position: relative;clear: all;overflow: hidden;padding: 5px;">
								应付金额：<span style="color: red;"><%=money %>元</span>
							</div>
						</li>
					</ul>
					<div class="mui-row card-tip">
						支持大额支付的<span onclick="showCardList()">《银行列表》</span>
					</div>
		...
		// 			alert(res.err_code + res.err_desc + res.err_msg);
	                	...
	                	alert("微信支付成功!");  
	                  	setTimeout(function(){//两秒后跳转  
                        	...
                        	//$.mobile.changePage("http://www.shmedifood.com/HomeController/maid"); //手机网页式跳转  
                       		window.location.href="<%=basePath%>HomeController/memberCenter";    
                		},20);  
			...
			var store_id = getUrlParam('store_id');
	      	...
	      	function showCardList(){
	      		var paramData={"1":"1"};                              
		      	var object = JSON.stringify(paramData);
		 		ajaxPost("<%=basePath%>weixin_bankManage/findAllBankManage",object,"json","json",getBankList);
		 	}
		 	
		 	function getBankList(data){
	 			var popInfoData = data.bankManageEntityData.bankManageEntityList;
	 			var bankListHtml = '';
	 			for(var i = 0;i < popInfoData.length;i++){
	 				
	 				bankListHtml += '<tr><td>'+ popInfoData[i].area +'</td><td>'+ popInfoData[i].bank_name +'</td><td>'+ setBankType(popInfoData[i].bank_type) +'</td><td>'+ popInfoData[i].limit_money +'</td></tr>';
	 			}
	 			var table = '<table>'+
	 						'<tr><th>区域</th><th>银行</th><th>卡类别</th><th>每笔/每日/每月</th></tr>'+bankListHtml +
	 						'</table>';
				var popInfo = {
					title:'银行卡列表',
					content:table
		 	...
		 	function setBankType(bank_type){
		 		var bankType = '信用卡';
				if(bank_type == 1){
					bankType = '储蓄卡';
				}
