...
<head>
  ...
  <script type="text/javascript">
	  ...
	  function callBack(vbm){
	     ...
	     for(var i=0;i<lbm.length;i++){
          ...
          htmlstr+="<tr>"+
                ...
                "<a onclick='examState("+lbm[i].bank_id+","+0+")' class='am-btn am-btn-default am-btn-xs am-text-secondary'><span class='am-icon-pencil-square-o'></span>通过</a>"+
                "<a onclick='examState("+lbm[i].bank_id+","+2+")' class='am-btn am-btn-default am-btn-xs am-text-secondary'><span class='am-icon-pencil-square-o'></span> 拒绝</a>"+ 
                "<a onclick='examState("+lbm[i].bank_id+","+3+")' class='am-btn am-btn-default am-btn-xs am-text-danger'><span class='am-icon-trash-o'></span> 删除</a>"+
                "</div>"+  
                "</div>"+    
                "</td>"+    
                "</tr> "; 
	 ...
	 // alert(11);
	  ...
	  ajaxPost("<%=basePath%>admin_examine/adminExamineByBankManage",object,"json","json",callBack);
	  };
	  ...
	  //通过审核    status_state=0
	  function examState(bank_id,status_state){
	  //debugger;
	  //alert(store_id);
	  var paramData;
	  ...
	  var object = JSON.stringify(paramData);
	  if(confirm("确认执行此操作吗？")){
	    ajaxPost("<%=basePath%>admin_examine/updateBankManageByStatus",object,"json","json",callBack1);
	    return true;
	  }else{
	    return false;
	    }
