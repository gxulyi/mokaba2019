...
<div class="add_style">
 <ul>
  <li class="clearfix"><label class="label_name col-xs-1"><i>*</i>标题：&nbsp;&nbsp;</label><div class="Add_content col-xs-11"><input style="width:180px;" name="title" type="text" data-title="标题" value="${sEntity.sowing_title}"  id="title" class="col-xs-6"/></div></li>
  <li class="clearfix"><label class="label_name col-xs-1">
       <i>*</i>类型：&nbsp;&nbsp;</label>
    <div class="Add_content col-xs-11">
      <select name="" id="selectidd" class="text_add" style="width:180px" class="col-xs-6" value="">
  ...
  <li class="clearfix"><label class="label_name col-xs-1"><i>*</i>顺序：&nbsp;&nbsp;</label><div class="Add_content col-xs-11"><input style="width:180px;" name="author" value="${sEntity.sort}" type="text"  data-author="顺序" id="sort"   class="col-xs-6"/></div></li>
  <li class="clearfix"><label class="label_name col-xs-1"><i>*</i>所属类型：&nbsp;&nbsp;</label>
    <div class="Add_content col-xs-11" id="radioId" >
    	<input type="hidden" id="store" value="${sEntity.sowing_type}"/>
       <input  type="radio" value="1" name="store" checked="checked" onclick="function_choose(1)">商品</input>  
       <input   type="radio" value="2"  name="store" onclick="function_choose(2)">知识动态</input>  
       <input   type="radio" value="3"  name="store" onclick="function_choose(3)">广告</input>  
     </div></li>
   ...
   <li class="clearfix"><label class="label_name col-xs-1"><i>*</i>类型名称：&nbsp;&nbsp;</label>
       <div class="am-btn-group am-btn-group-xs">
            ...
            <div class="am-form-group am-margin-left am-fl" >
              <input type="hidden" id="sowingMode1" value="${sEntity.type_id}"/>
              <select id="sowingMode" name="sowingMode" onchange="showStore(this.value)" style="width:180px;">
            ...
            <div id="uploader">
                ...
                <div id="sowingPic">
                    ...
                    <div id="dndArea" class="placeholder">
                        ...
                        <input id="pictu" value="${sEntity.sowing_picture}">
                        <p>或将照片拖到这里，只需要一张</p>
                    </div>
                </div>
                <input id="sowing_picture" value="" type="hidden">
 ...
 <div class="Button_operation btn_width">
    <button class="btn button_btn bg-deep-blue"  style="margin-left: 30%;" type="button" onclick="updateSowing()">保存并提交</button>
  ...
  <script type="text/javascript">
	 ...
	 function onloadd(){
	      debugger;
	        /*获取轮播图类型*/
	       document.getElementById("selectidd").value = '${sEntity.sowing_mode}';
		    /*通过后台传值给单选框按钮选中，类似的下拉框也可以这样做*/
	       $(function(){
			    var store= $("#store").val();
			    var sowingMode= $("#sowingMode1").val();
			    if(!isNull(store)){
			        $("input:radio[name='store'][value="+store+"]").attr('checked','true');
			        function_choose(store);
			    }
			});
		  ...
		  var pic=pictures[i];
		  	str+=
		  	"<img alt=''  src='<%=basePath%>file/sowing/img/"+pic+"'>";
		  }
		  document.getElementById("sowingPic").innerHTML=str;
	...
	function function_choose(value){
		 ...
		 if(value==1){
		    ...
		    ajaxPost("<%=basePath%>admin_news/findNewsByState",object,"json","json",showStore);
		 }else if(value==3){
		    ajaxPost("<%=basePath%>admin_article/findAllArticle",object,"json","json",showStore);
		 } 
	}
	/*回调函数获取数据*/
	function showStore(vsi){
	   //alert(JSON.stringify(vsi));
	    var htmlstr2="";
	    if(vsi.object=="知识动态"){
     	  var newsData=vsi.newsDate.listNews;
	      if(newsData!=null){
	      var nd=newsData;
	      for(var i=0;i<nd.length;i++){
           htmlstr2+="<option   value="+nd[i].news_id+">"+nd[i].news_title+"</option>";
		  }
		  document.getElementById("sowingMode").innerHTML=htmlstr2;
		  /*所属类型获取下拉框的值并选中*/
		  var sowingMode=$("#sowingMode1").val(); 
			   //alert(JSON.stringify(sowingMode)); 
			   if(!isNull(sowingMode)){
			        $("#sowingMode option[value='"+sowingMode+"']").prop("selected", "selected");
			    }
		  	
			  }else{
			  	alert("暂无服务内容信息");
			  }
	    }else if(vsi.object=="商品"){
	      var StoreBasicsData=vsi.sbd;
     	  var storeBasicsList=StoreBasicsData.storeBasicsList;
	      if(storeBasicsList!=null){
	      var sbl=storeBasicsList;
	      for(var i=0;i<sbl.length;i++){
           htmlstr2+="<option  value="+sbl[i].store_id+">"+sbl[i].store_name+"</option>";
		  }
		  document.getElementById("sowingMode").innerHTML=htmlstr2;
		   /*所属类型获取下拉框的值并选中*/
		   var sowingMode=$("#sowingMode1").val(); 
	    ...
	    }else if(vsi.object=="广告"){
	      var articleEntityList=vsi.articleEntityData.articleEntityList;
	      if(articleEntityList!=null){
	      var sbl=articleEntityList;
	      for(var i=0;i<sbl.length;i++){
           htmlstr2+="<option  value="+sbl[i].article_id+">"+sbl[i].title+"</option>";
		  }
		  document.getElementById("sowingMode").innerHTML=htmlstr2;
		   /*所属类型获取下拉框的值并选中*/
		   var sowingMode=$("#sowingMode1").val(); 
    ...
    function updateSowing(){
    	  debugger;
    	  /*获取请求参数*/
    	  var sowing_id = '${sEntity.sowing_id}';
	      var sowing_title=document.getElementById("title").value;
	      var sowing_mode=document.getElementById("selectidd").value;
	      var type_id =$.trim($("#sowingMode").val());
	      var sowing_type= $("input[name='store']:checked").val();//获取单选框的值
          var sort= document.getElementById("sort").value; 
	      var sowing_picture=document.getElementById("sowing_picture").value;
	      var sowing_pictures=document.getElementById("pictu").value;
	      var picture =sowing_pictures+";"+sowing_picture;
	       /*校验表单数据*/
	      if(sowing_title ==null ||sowing_title==""){
	        alert("请输入标题");
	        return;
	      }
	      var reg =/^[0-9]*$/; 
	      if((sort == null || sort == "")){
	       ...
	       if(!reg.test(sort)){
	        alert("请输入数字，轮播图顺序只能是数字");
	        return;
	       }
	      }
	     /*  if(sowing_type ==null ||sowing_type==""){
	        alert("请选择所属类型");
	        return;
	      }*/
	       
	      var paramData;
	 	  var paramData={sowing_id:sowing_id,type_id:type_id,sowing_title:sowing_title,sowing_mode:sowing_mode,sowing_type:sowing_type,sort:sort,sowing_picture:picture};                               
	  	  var object = JSON.stringify(paramData);
	  	  //alert(object);
	 	  if(confirm("确认执行这个操作吗？")){
	        ajaxPost("<%=basePath%>/admin_sowing/updateSowing",object,"json","json",wait);
	  }else{
	    return false;
	    }
   ...
   function wait(data){
    ...
    $(function() {
        ...
        uploader = WebUploader.create({
            ...
            chunked: true,
            chunkSize: 512 * 1024,
            server: 'UEditorTemplateController/sowingFildUpload',
            method:'POST',
            // runtimeOrder: 'flash',
    ...
    /*  uploader.on( 'uploadSuccess', function( file,response ) { 
     ...
     var sowing_picture=document.getElementById("sowing_picture").value;
     if(sowing_picture==""){
     	document.getElementById("sowing_picture").value=imageName;
     }else{
     	sowing_picture+=";"+imageName;
     	document.getElementById("sowing_picture").value=sowing_picture;
     }
    });
