...
<script type="text/javascript">
	  ...
	  function wait(vnl){
	     ...
	     if(object[i].recommend_type==1){
	         recommend_type = "微信扫上级推荐码";
	     }else if(object[i].recommend_type==2){
	         recommend_type = "后台创建";
	     }else{
	         recommend_type = "暂无信息";
	     }
	     if(object[i].superior_information_compellation==null){
	         teamName="未知";
	     }else if(object[i].superior_information_compellation!=null){
	         teamName=object[i].superior_information_compellation;
	     }
          htmlstr+="<tr>"+
                "<td>"+object[i].information_compellation+"</td>"+
                "<td>"+object[i].team_num+"</td>"+
                "<td><a style='cursor:pointer' onclick='findTeamId("+object[i].user_basics_id+")'>"+object[i].direct_team_num+"</a></td>"+
                "<td>"+object[i].records_integral+"</td>"+
                "<td>￥"+object[i].records_bonus.toFixed(2)+"</td>"+
                "<td>"+object[i].team_consume+"</td>"+
                "<td>"+object[i].direct_team_consume+"</td>"+
                "<td>"+teamName+"</td>"+
	  ...
	  window.onloadd=function(){
	    ...
	    ajaxPost("<%=basePath%>admin_user/findTeamId",object,"json","json",wait);
	  };
