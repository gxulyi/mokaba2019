	  ...
	  var htmlstr="";
	     ...
	     for(var i=0;i<lst.length;i++){
          ...
          htmlstr+="<tr>"+
                ...
                "<a onclick='findExpertConsultById("+lst[i].consult_id+")' class='am-btn am-btn-default am-btn-xs am-text-secondary'><span class='am-icon-pencil-square-o'></span>查看</a>"+ 
                "<a onclick='updateExpertConsult("+lst[i].consult_id+")' class='am-btn am-btn-default am-btn-xs am-text-secondary'><span class='am-icon-pencil-square-o'></span> 回复</a>"+
/*                 "<a onclick='deleteExpertConsult("+lst[i].consult_id+","+3+")' class='am-btn am-btn-default am-btn-xs am-text-danger'><span class='am-icon-trash-o'></span> 删除</a>"+
 */                "</div>"+  
                "</div>"+    
                "</td>"+    
                "</tr> "; 
	  ...
	  ajaxPost("<%=basePath%>admin_expertConsult/findAllExpertConsult",object,"json","json",wait);
	  };
	  ...
	  //查询专家咨询详情
	  function findExpertConsultById(consult_id){
	  var parame="?consult_id="+consult_id+"&expert_state="+0;
	  //alert(parame);
	  window.location="<%=basePath%>admin_expertConsult/toAdminFindByIdExpertConsult"+parame;
	  };
	  
	  
	  //修改专家咨询
	  function updateExpertConsult(consult_id){
	  var parame="?consult_id="+consult_id+"&expert_state="+0;
	  //alert(parame);
	  window.location="<%=basePath%>admin_expertConsult/toAdminUpdateExpertConsult"+parame;
	  };
	  
	  //删除专家咨询
	  function deleteExpertConsult(consult_id,status_state){
	  debugger;
	  var paramData;
	  var paramData={consult_id:consult_id,status_state:status_state};                               
	  var object = JSON.stringify(paramData);
	  if(confirm("确认提交操作吗？")){
	   ajaxPost("<%=basePath%>admin_expertConsult/deleteExpertConsultById",object,"json","json",wait2);
	  }else{
	    return false;
	    }
	  };
	  ...
	  //根据选择查询
	  function selectExpertConsultByExpertState(){
	    var paramData;
        var expert_state= $("#selectid").find("option:selected").val();
	    var paramData={expert_state:expert_state};                               
	    var object = JSON.stringify(paramData);
        ajaxPost("<%=basePath%>admin_expertConsult/findExpertConsultByExpertState",object,"json","json",wait);
