...
<script type="text/javascript" src="js/jquery-1.7.1.js"></script>
  ...
  <script type="text/javascript">
    ...
    function deleteNew(news_id){
	  ...
	  if(confirm("确定删除？")){
	    	ajaxPost("<%=basePath%>admin_news/deleteEssence",object,"json","json",wait2);
	  	}else{
	    	return false;
	    }
    }
    ...
    function wait2(data){
	  	window.location="<%=basePath%>admin_news/toFindEssence";
    }
	...
	function wait(data){
  		...
  		if(newsDate!=null){
  			...
  			htmlstr +=  "<tr>"+
                    	...
                    	"				<a onclick='findEvaluation("+newsDate[i].news_id+")' class='am-btn am-btn-default am-btn-xs am-text-danger'>查看评论</a>"+
                  		"			</div>"+
                		"		</div>"+
              			"	</td>"+
            			"</tr>";
	  ...
	  var object = JSON.stringify(paramData);
	  ajaxPost("<%=basePath%>admin_news/findEssence",object,"json","json",wait);
	  };
	  window.onload=function(){
	  	onloadd();
	  };
	  function findEvaluation(news_id){
	  	var parame="?news_id="+news_id;
	    window.location="<%=basePath%>admin_news/toFindEssenceEvaluation"+parame;
	  };
    </script>
      ...
      <div class="am-u-md-6 am-cf">
        <div class="am-fl am-cf">
          <div class="am-btn-toolbar am-fl">
            <div class="am-btn-group am-btn-group-xs">
           <!--    <a href="admin_news/toAddKnow"><button type="button" class="am-btn am-btn-default"><span class="am-icon-plus"></span> 新增</button></a>     -->
            </div>

            <div class="am-form-group am-margin-left am-fl">
              <select>
                <option value="option1">所有类别</option>
                <option value="option2">知识类</option>
                <option value="option3">动态类</option>
              </select>
            </div>
          </div>
        </div>
          ...
          <table id="yhk" class="am-table am-table-striped am-table-hover table-main">
            <thead>
              <tr>
                ...
                <th class="table-id">精华编号</th>
                <th class="table-title">精华名称</th>
